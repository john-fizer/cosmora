...
const MOON_PHASES = [
  { name: "New Moon",        emoji: "🌑", min: 0,   max: 1.5  },
  { name: "Waxing Crescent", emoji: "🌒", min: 1.5, max: 24   },
  { name: "First Quarter",   emoji: "🌓", min: 24,  max: 26   },
  { name: "Waxing Gibbous",  emoji: "🌔", min: 26,  max: 49   },
  { name: "Full Moon",       emoji: "🌕", min: 49,  max: 51   },
  { name: "Waning Gibbous",  emoji: "🌖", min: 51,  max: 74   },
  { name: "Last Quarter",    emoji: "🌗", min: 74,  max: 76   },
  { name: "Waning Crescent", emoji: "🌘", min: 76,  max: 98.5 },
  { name: "New Moon",        emoji: "🌑", min: 98.5,max: 100  },
] as const;
...
function MoonPhaseWidget() {
  ...
  // SVG moon: 48×48, draw crescent/gibbous using overlapping circles
  const R = 20, CX = 24, CY = 24;
  // illumination 0→100 maps to phase shape
  // waxing: right side lit; waning: left side lit
  const lit = illumination / 100;
  // Shadow circle offset: at 0% → full shadow (circle on top), at 50% → edge-on, at 100% → full lit
  ...
  return (
    <div className="rounded-2xl p-4"
      ...
      <p className="text-[8px] font-bold tracking-widest mb-3" style={{ color: "#64748b" }}>MOON PHASE</p>
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold leading-tight" style={{ color: "#c4b5fd" }}>{phase.name}</p>
          <p className="text-[9px] mt-0.5 tabular-nums" style={{ color: "#475569" }}>
          ...
          <p className="text-[8px] mt-1.5 tabular-nums" style={{ color: "#334155" }}>
...
function PlanetaryHourWidget({ lat, lon }: { lat: number; lon: number }) {
  ...
  return (
    <div className="rounded-2xl p-4"
      ...
      <p className="text-[8px] font-bold tracking-widest mb-3" style={{ color: "#64748b" }}>PLANETARY HOUR</p>
      <div className="flex items-center gap-3">
        <svg width={72} height={72} viewBox="0 0 72 72">
          ...
          <text x={CX} y={CY - 4} textAnchor="middle" dominantBaseline="central"
            fontSize={20} fill={color}>{PLANET_SYMBOLS[curr.planet as PlanetName] ?? "✦"}</text>
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold leading-tight" style={{ color }}>{curr.planet}</p>
          <p className="text-[8px] tabular-nums" style={{ color: "#475569" }}>
          ...
          <div className="flex gap-1 mt-2">
            {upcoming.map((h, i) => (
              <div key={i} className="flex flex-col items-center gap-0.5 px-1.5 py-1 rounded-lg"
                ...
                <span className="text-[11px] leading-none" style={{ color: CHALDEAN_COLORS[h.planet] }}>
                  {PLANET_SYMBOLS[h.planet as PlanetName] ?? "?"}
                </span>
                <span className="text-[6px] tabular-nums" style={{ color: "#334155" }}>
...
function CosmicClock() {
  ...
  return (
    <div className="text-center">
      <p className="text-xs font-bold tracking-widest" style={{ color: "#06b6d4", fontFamily: "'Share Tech Mono', monospace" }}>
        {time.toLocaleTimeString("en-US", { hour12: true, hour: "numeric", minute: "2-digit", second: "2-digit" })}
      </p>
      <p className="text-[7px] tracking-widest" style={{ color: "#334155" }}>
    ...
    <div className="space-y-1.5">
      {(Object.entries(counts) as [keyof typeof counts, number][]).map(([el, count]) => (
        <div key={el} className="flex items-center gap-2">
          <span className="text-[8px] font-bold w-8 flex-shrink-0" style={{ color: ELEMENT_COLORS[el] }}>{el.substring(0, 4)}</span>
          <div className="flex-1 h-1 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
            <motion.div
              initial={{ width: 0 }}
          ...
          <span className="text-[8px] w-6 text-right" style={{ color: "#475569" }}>{Math.round((count / total) * 100)}%</span>
        </div>
      ))}
    </div>
  );
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-bold" style={{ color: "#e2e8f0" }}>{planet.name}{planet.retrograde ? " ℞" : ""}</p>
              <p className="text-[8px] tracking-widest" style={{ color: "#475569" }}>NATAL</p>
            </div>
          </div>
          <button onClick={onClose} className="text-lg leading-none cursor-pointer" style={{ color: "#475569" }}>×</button>
        </div>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          className="w-full py-2 rounded-xl text-[8px] font-bold tracking-widest cursor-pointer"
          style={{ background: `${color}10`, border: `1px solid ${color}25`, color }}
        >
          FULL {planet.name.toUpperCase()} READING →
        </motion.button>
      </div>
    </motion.div>
  );
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-1.5">
        ...
        />
        <span className="text-[7px] font-bold tracking-[0.2em]" style={{ color: "#334155" }}>LIVE TRANSITS</span>
      ...
      >
        {top.map((a, i) => {
          ...
          return (
            ...
            >
              <span className="text-xs leading-none" style={{ color: tColor }}>{PLANET_SYMBOLS[a.transitPlanet]}</span>
              {a.transitRetrograde && <span className="text-[7px]" style={{ color: "#f97316" }}>℞</span>}
              <span className="text-[10px] font-bold mx-0.5" style={{ color: aColor }}>{ASPECT_GLYPHS[a.type]}</span>
              <span className="text-xs leading-none" style={{ color: nColor }}>{PLANET_SYMBOLS[a.natalPlanet]}</span>
              <span className="ml-1 text-[7px]" style={{ color: "#334155" }}>H{a.natalHouse}</span>
              {a.exact && (
                <span className="ml-0.5 text-[6px] font-bold px-1 rounded" style={{ background: "rgba(167,139,250,0.2)", color: "#a78bfa" }}>EX</span>
              )}
              {a.applying && !a.exact && (
                <span className="ml-0.5 text-[6px] font-bold" style={{ color: "#22c55e" }}>→</span>
              )}
            </motion.div>
          );
        })}
...
function DailyBriefing({ chart, transits, profileId }: {
  ...
  useEffect(() => {
    ...
    const top3 = transits.slice(0, 3).map(t =>
      `${t.transitPlanet}${t.transitRetrograde ? " Rx" : ""} ${t.type} natal ${t.natalPlanet} H${t.natalHouse} (${t.orb.toFixed(1)}°${t.applying ? ", applying" : ", separating"})`
    ).join("; ");
...
2–3 sentences. Grounded, specific, evocative. Do not start with "Today". No bullet points.`;
        ...
        <div className="px-5 py-4">
          {!text && streaming ? (
            <div className="flex items-center gap-3">
              ...
              />
              <span className="text-[10px]" style={{ color: "#334155" }}>Reading the skies…</span>
            </div>
          ) : (
          ...
          >
            {transits.slice(0, 4).map((t, i) => {
              ...
              return (
                ...
                >
                  <span className="text-xs leading-none" style={{ color: tColor }}>{PLANET_SYMBOLS[t.transitPlanet]}</span>
                  <span className="text-[10px] font-bold" style={{ color: aColor }}>{ASPECT_GLYPHS[t.type]}</span>
                  <span className="text-xs leading-none" style={{ color: PLANET_COLORS[t.natalPlanet] ?? "#94a3b8" }}>{PLANET_SYMBOLS[t.natalPlanet]}</span>
                  <span className="text-[7px] font-mono ml-0.5" style={{ color: "#334155" }}>{t.orb.toFixed(1)}°</span>
                </div>
              );
            })}
          </div>
    ...
    >
      ...
      >
        <div className="px-4 pt-3 pb-2" style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
          <p className="text-[8px] font-bold tracking-widest" style={{ color: "#7c3aed" }}>PLANETARY POSITIONS</p>
        </div>
        <div className="flex-1 overflow-y-auto" style={{ maxHeight: 260, scrollbarWidth: "none" }}>
          {chart.planets.map((p) => {
            ...
            return (
              ...
              >
                <span className="text-sm w-5 flex-shrink-0" style={{ color }}>{PLANET_SYMBOLS[p.name]}</span>
                <span className="text-[9px] font-medium flex-1" style={{ color: "#94a3b8" }}>
                  {p.name}{p.retrograde ? " ℞" : ""}
                </span>
                <span className="text-[9px] font-bold" style={{ color: signColor }}>
                  {p.signDegree.toFixed(0)}° {SIGN_SYMBOLS[p.sign]}
                </span>
                <span className="text-[8px] w-6 text-right" style={{ color: "#334155" }}>H{p.house}</span>
              </motion.div>
            );
          })}
        </div>
      ...
      >
        <div className="px-4 pt-3 pb-2" style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
          <p className="text-[8px] font-bold tracking-widest" style={{ color: "#06b6d4" }}>ASPECTS</p>
        </div>
          ...
          {chart.aspects.slice(0, 7).map((a, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-1.5 text-[9px]">
                ...
                <span style={{ color: "#334155" }}>{a.orb.toFixed(1)}°</span>
                {a.exact && <span className="text-[7px]" style={{ color: "#22c55e" }}>EX</span>}
              </div>
            );
          })}
        </div>
        <div className="px-3 pb-3">
          <Link href="/dashboard/chart">
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
              className="w-full py-1.5 rounded-lg text-[8px] font-bold tracking-widest cursor-pointer"
...
function computeConvergence(chart: ChartData, transits: TransitPill[]): {
  ...
  // Factor 2: Outer planet transits to Lord of Year (applying, ≤3°)
  ...
  if (towardLord.length >= 2) { score += 32; factors.push({ text: `${towardLord.length}× outer transits to ${lord}`, pts: 32 }); }
    ...
    score += pts;
    factors.push({ text: `${towardLord[0].transitPlanet}→${lord} (${towardLord[0].orb.toFixed(1)}°)`, pts });
  }
...
function ConvergenceWidget({ chart, transits }: { chart: ChartData; transits: TransitPill[] }) {
  ...
  return (
    <Link href="/dashboard/insights">
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <p className="text-[8px] font-bold tracking-widest" style={{ color }}>CONVERGENCE</p>
          <span className="text-[7px] tracking-widest" style={{ color: "#1e293b" }}>TIMING ALIGNMENT</span>
        </div>
          ...
          <div className="flex flex-col gap-1.5 flex-1 min-w-0">
            {factors.slice(0, 3).map((f, i) => (
              ...
              >
                ...
                <span className="text-[8px] leading-snug" style={{ color: "#475569" }}>{f.text}</span>
                <span className="ml-auto text-[7px] font-bold flex-shrink-0" style={{ color }}>+{f.pts}</span>
              </motion.div>
            ))}
            {factors.length === 0 && (
              <span className="text-[8px]" style={{ color: "#334155" }}>No major alignments active right now.</span>
            )}
          </div>
        </div>

        <div className="mt-2.5 pt-2" style={{ borderTop: "1px solid rgba(255,255,255,0.04)" }}>
          <span className="text-[7px] font-bold tracking-widest" style={{ color: "#334155" }}>
            VIEW FULL ANALYSIS →
          </span>
        </div>
      </motion.div>
...
function SkyStatusWidget({ retrogrades, ingresses }: { retrogrades: PlanetName[]; ingresses: Ingress[] }) {
  ...
  return (
    ...
    >
      <p className="text-[8px] font-bold tracking-widest mb-3" style={{ color: "#475569" }}>SKY STATUS</p>

      {retrogrades.length > 0 && (
        <div className="mb-3">
          <p className="text-[7px] tracking-widest mb-1.5" style={{ color: "#1e293b" }}>RETROGRADE</p>
          <div className="flex flex-wrap gap-1.5">
            {retrogrades.map(name => {
              ...
              return (
                ...
                >
                  <span style={{ fontSize: "0.75rem", color }}>{PLANET_SYMBOLS[name] ?? "○"}</span>
                  <span className="text-[8px] font-bold" style={{ color }}>{name.slice(0, 3)}</span>
      ...
      {ingresses.length > 0 && (
        <div>
          <p className="text-[7px] tracking-widest mb-1.5" style={{ color: "#1e293b" }}>NEXT INGRESS</p>
          <div className="space-y-1.5">
            {ingresses.slice(0, 3).map((ing, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-2">
                  ...
                  <span className="text-[8px] font-medium" style={{ color: "#94a3b8" }}>{SIGN_SYMBOLS[ing.toSign]} {ing.toSign}</span>
                  <span className="text-[8px] font-mono ml-auto" style={{ color: "#334155" }}>
...
function NextExactWidget({ transits }: { transits: TransitPill[] }) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <p className="text-[8px] font-bold tracking-widest" style={{ color: "#06b6d4" }}>NEXT EXACT</p>
        {exact.length > 0 && (
          <motion.div
            animate={{ opacity: [1, 0.3, 1] }}
          ...
          >
            ...
            <span className="text-[7px] font-bold tracking-widest" style={{ color: "#22c55e" }}>EXACT NOW</span>
          </motion.div>
        )}
      </div>
      ...
      {topExact && (
        <div className="flex items-center gap-2 mb-2 p-2 rounded-xl" style={{ background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.2)" }}>
          <span style={{ fontSize: "1rem", color: tColor(topExact.transitPlanet) }}>
            {PLANET_SYMBOLS[topExact.transitPlanet] ?? "○"}
          </span>
          <span className="text-sm font-bold" style={{ color: ASPECT_COLORS[topExact.type] ?? "#94a3b8" }}>
            {ASPECT_GLYPHS_NE[topExact.type] ?? "~"}
          </span>
          <span style={{ fontSize: "1rem", color: tColor(topExact.natalPlanet) }}>
            {PLANET_SYMBOLS[topExact.natalPlanet] ?? "○"}
          </span>
          <div className="flex-1 min-w-0">
            <p className="text-[8px] font-semibold truncate" style={{ color: "#94a3b8" }}>
              {topExact.transitPlanet} {topExact.type} {topExact.natalPlanet}
            </p>
          </div>
          <span className="text-[7px] font-bold px-1.5 py-0.5 rounded" style={{ background: "rgba(34,197,94,0.15)", color: "#22c55e" }}>EXACT</span>
        </div>
      )}
      ...
      {next && (
        <div className="flex items-center gap-2">
          <span style={{ fontSize: "1rem", color: tColor(next.transitPlanet) }}>
            {PLANET_SYMBOLS[next.transitPlanet] ?? "○"}
          </span>
          <span className="text-sm font-bold" style={{ color: ASPECT_COLORS[next.type] ?? "#94a3b8" }}>
            {ASPECT_GLYPHS_NE[next.type] ?? "~"}
          </span>
          <span style={{ fontSize: "1rem", color: tColor(next.natalPlanet) }}>
            {PLANET_SYMBOLS[next.natalPlanet] ?? "○"}
          </span>
          <div className="flex-1 min-w-0">
            <p className="text-[8px] font-semibold truncate" style={{ color: "#94a3b8" }}>
          ...
          {next.daysToExact != null && (
            <span className="text-[8px] font-bold font-mono flex-shrink-0" style={{ color: "#06b6d4" }}>
      ...
      {applying.length > 1 && (
        <div className="mt-2 pt-2" style={{ borderTop: "1px solid rgba(255,255,255,0.04)" }}>
          <p className="text-[7px] tracking-widest mb-1.5" style={{ color: "#1e293b" }}>APPROACHING</p>
          <div className="flex flex-wrap gap-1">
            {applying.slice(1, 4).map((t, i) => (
              <div key={i} className="flex items-center gap-1 px-2 py-1 rounded-lg" style={{ background: "rgba(6,182,212,0.06)", border: "1px solid rgba(6,182,212,0.1)" }}>
                <span style={{ fontSize: "0.7rem", color: tColor(t.transitPlanet) }}>{PLANET_SYMBOLS[t.transitPlanet] ?? "○"}</span>
                <span style={{ fontSize: "0.65rem", color: ASPECT_COLORS[t.type] ?? "#64748b" }}>{ASPECT_GLYPHS_NE[t.type] ?? "~"}</span>
                <span style={{ fontSize: "0.7rem", color: tColor(t.natalPlanet) }}>{PLANET_SYMBOLS[t.natalPlanet] ?? "○"}</span>
                {t.daysToExact != null && (
                  <span className="text-[7px] font-mono" style={{ color: "#334155" }}>{fmtDays(t.daysToExact)}</span>
                )}
              </div>
            ))}
          </div>
...
function RightPanel({ chart, profileId, lat, lon, transits, retrogrades, ingresses }: { chart: ChartData; profileId: string | null; lat: number; lon: number; transits: TransitPill[]; retrogrades: PlanetName[]; ingresses: Ingress[] }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <p className="text-[8px] font-bold tracking-widest" style={{ color: "#7c3aed" }}>COSMIC INSIGHT</p>
          <span className="text-[7px] tracking-widest" style={{ color: "#334155" }}>AI ANALYSIS</span>
        </div>
        <div className="flex items-start gap-2 mb-3">
          ...
          <div>
            <p className="text-[8px] font-bold tracking-wider mb-1" style={{ color: "#a78bfa" }}>
              {prof.lordOfYear.toUpperCase()} YEAR · H{prof.activatedHouse}
            </p>
      ...
      <Link href="/dashboard/briefing">
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-[8px] font-bold tracking-widest" style={{ color: "#f59e0b" }}>TODAY'S BRIEFING</p>
          </div>
          <p className="text-[9px] leading-relaxed mb-3" style={{ color: "#64748b" }}>
            Morning cosmic weather, planetary hours &amp; AI-narrated sky report.
          </p>
          <div className="text-[8px] font-bold tracking-widest" style={{ color: "#f59e0b" }}>
            OPEN BRIEFING →
          </div>
        </motion.div>
      </Link>
      ...
      <Link href="/dashboard/report">
        ...
        >
          ...
          <div className="text-[8px] font-bold tracking-widest" style={{ color: "#c4b5fd" }}>
            GENERATE REPORT →
          </div>
        </motion.div>
      </Link>
      ...
      >
        <p className="text-[8px] font-bold tracking-widest mb-3" style={{ color: "#64748b" }}>KEY PLACEMENTS</p>
          ...
          {[
            ...
            return (
              <div key={label} className="flex items-center gap-2">
                <span className="text-[7px] font-bold tracking-widest w-10 flex-shrink-0" style={{ color: "#334155" }}>{label}</span>
                <span className="text-base leading-none" style={{ color: SIGN_COLORS[displaySign] }}>{SIGN_SYMBOLS[displaySign]}</span>
                <span className="text-[9px] font-medium" style={{ color }}>{displaySign}</span>
                {planet && <span className="text-[8px] ml-auto" style={{ color: "#334155" }}>H{planet.house}</span>}
              </div>
            );
          })}
        </div>
        <div className="mt-3 pt-3 flex items-center gap-2" style={{ borderTop: "1px solid rgba(255,255,255,0.04)" }}>
          <span className="text-[7px] font-bold tracking-widest" style={{ color: "#334155" }}>SECT</span>
          <span className="text-[9px] font-bold" style={{ color: chart.sect === "day" ? "#fbbf24" : "#c4b5fd" }}>
            {chart.sect === "day" ? "☀ DAY" : "☽ NIGHT"}
          </span>
          <span className="ml-auto text-[7px] font-bold tracking-widest" style={{ color: "#334155" }}>DOMINANT</span>
          <span className="text-[9px] font-bold" style={{ color: ELEMENT_COLORS[dominant as keyof typeof ELEMENT_COLORS] }}>{dominant}</span>
        </div>
      </div>
      ...
      >
        <p className="text-[8px] font-bold tracking-widest mb-3" style={{ color: "#64748b" }}>ENERGY BALANCE</p>
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      >
        ...
        <text x={70} y={154} textAnchor="middle" fontSize={11} fill={sunColor}>{sun?.sign ?? "—"}</text>
        <text x={70} y={168} textAnchor="middle" fontSize={8} fill="rgba(251,191,36,0.5)">{sun ? `${sun.signDegree.toFixed(0)}° H${sun.house}` : ""}</text>
        ...
        <text x={180} y={154} textAnchor="middle" fontSize={11} fill={moonColor}>{moon?.sign ?? "—"}</text>
        <text x={180} y={168} textAnchor="middle" fontSize={8} fill="rgba(196,181,253,0.5)">{moon ? `${moon.signDegree.toFixed(0)}° H${moon.house}` : ""}</text>

        {/* Rising */}
        <text x={290} y={118} textAnchor="middle" fontSize={26} fill={ascColor}>↑</text>
        <text x={290} y={140} textAnchor="middle" fontSize={8} fill={ascColor} fontWeight="700" letterSpacing={1}>RISING</text>
        <text x={290} y={154} textAnchor="middle" fontSize={11} fill={ascColor}>{asc?.sign ?? "—"}</text>
        <text x={290} y={168} textAnchor="middle" fontSize={8} fill="rgba(6,182,212,0.5)">{asc ? `${asc.signDegree.toFixed(0)}°` : ""}</text>

        {/* Lord of Year */}
        <text x={390} y={108} textAnchor="middle" fontSize={26} fill={lordColor} filter="url(#sigGlow)">
          {PLANET_SYMBOLS[prof.lordOfYear] ?? "✦"}
        </text>
      ...
      <div className="flex items-center gap-3">
        <motion.button
          ...
          className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold tracking-widest cursor-pointer"
        ...
        <p className="text-[9px]" style={{ color: "#334155" }}>Screenshot works too</p>
      </div>
    </div>
  );
}
...
export default function DashboardPage() {
  ...
  if (!loading && !profileId) {
    return (
      <div className="h-screen flex flex-col items-center justify-center gap-6" style={{ background: "#00000f" }}>
        <div className="w-20 h-20 rounded-2xl flex items-center justify-center"
          style={{ background: "linear-gradient(135deg, #7c3aed, #06b6d4)", boxShadow: "0 0 50px rgba(124,58,237,0.6)" }}>
          <svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.2" className="w-10 h-10">
            <circle cx="12" cy="12" r="9" /><circle cx="12" cy="12" r="3" />
          </svg>
        </div>
        <div className="text-center space-y-2">
          <h2 className="text-2xl font-bold" style={{ color: "#e2e8f0" }}>No chart found</h2>
          <p className="text-sm" style={{ color: "#64748b" }}>Enter your birth data to begin.</p>
        </div>
        <Link href="/onboarding">
          <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
            className="px-8 py-3 rounded-xl text-sm font-bold tracking-wider cursor-pointer"
  ...
  return (
    <div className="relative h-screen overflow-hidden" style={{ background: "#00000f" }}>
      ...
      <div className="relative z-10 h-full flex flex-col pointer-events-none">
        ...
        >
          ...
          <Link href="/">
            <div className="flex items-center gap-2 cursor-pointer">
              ...
              <div className="hidden md:block">
                <p className="text-[9px] font-bold tracking-[0.2em]" style={{ color: "#7c3aed" }}>COSMORA</p>
                <p className="text-[7px] tracking-widest" style={{ color: "#334155" }}>2070</p>
              </div>
            </div>
          </Link>
          ...
          <div className="flex-1 flex items-center justify-center gap-6">
            ...
            <CosmicClock />
            {chart && (
              <span className="text-[9px] tracking-widest hidden md:block" style={{ color: "#334155" }}>
                BIRTH CHART
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            <Link href="/dashboard/chart">
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[9px] font-bold tracking-widest cursor-pointer"
            ...
            {chart && (
              <motion.button
                ...
                className="hidden md:flex items-center gap-1 px-3 py-1.5 rounded-lg text-[9px] font-bold tracking-widest cursor-pointer"
            ...
            <Link href="/onboarding">
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-[9px] font-bold tracking-widest cursor-pointer"
        ...
        <div className="flex-1 flex items-center px-4 md:px-6 gap-4 overflow-hidden min-h-0">
          ...
          {/* Center — chart + planet popup + daily briefing */}
          ...
          >
            ...
            >
              <button
                ...
                className="absolute -top-10 right-0 text-sm cursor-pointer font-bold tracking-widest"
                style={{ color: "#475569" }}
              >
                ✕ CLOSE
              </button>
